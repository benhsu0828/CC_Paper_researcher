...
def _rt(text: str) -> list[dict]:
    """把文字切成 ≤1900 字的 rich_text 片段（Notion 單片段上限 2000）。順手轉繁體。"""
    text = to_traditional(text or "")
    out = []
    for i in range(0, len(text), 1900):
        out.append({"type": "text", "text": {"content": text[i:i + 1900]}})
    return out or [{"type": "text", "text": {"content": ""}}]


def _paras(text: str) -> list[dict]:
    """長文字 → 多個 paragraph block（每塊 ≤1900 字）。順手轉繁體。"""
...
def _bullets(items: list) -> list[dict]:
    ...
    for it in items or []:
        ...
        out.append({"object": "block", "type": "bulleted_list_item",
                    "bulleted_list_item": {"rich_text": _rt(s[:1900])}})
    return out


def _callout(text: str, emoji: str = "🔥") -> dict:
    return {"object": "block", "type": "callout", "callout": {
        "rich_text": _rt(text[:1900]), "icon": {"type": "emoji", "emoji": emoji},
