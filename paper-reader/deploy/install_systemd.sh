...
# 預設專案位於 ~/CC_Paper_researcher/paper-reader；若 clone 到別處，請改 unit 檔的路徑。
set -euo pipefail

DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
DEST="$HOME/.config/systemd/user"

mkdir -p "$DEST" "$HOME/CC_Paper_researcher/paper-reader/data/logs"
...
echo "或檔案日誌：    tail -f ~/CC_Paper_researcher/paper-reader/data/logs/nightly.log"
